...
$message = '';
// Action: Save status updates submitted for a declaration.
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $id = (int) ($_POST['id'] ?? 0);
    $status = $_POST['status'] ?? '';
    if ($id > 0 && in_array($status, ['draft', 'submitted'], true)) {
        // Action: Update declaration status in database.
...
// Action: Load declarations list with optional status filter and selected order.
$stmt = $pdo->prepare(
...
?>
<!doctype html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Manage Submissions</title><link rel="stylesheet" href="../assets/css/header.css"><link rel="stylesheet" href="../assets/css/admin.css"><link rel="stylesheet" href="../assets/css/footer.css"></head><body>
